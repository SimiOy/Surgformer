...
class PhaseDataset_M2CAI16(PhaseDataset_Cholec80):
    """M2CAI16 phase dataset.

    Identical to PhaseDataset_Cholec80 except frames are stored in per-video
    HDF5 files (one file per video) instead of individual PNGs.

    HDF5 layout (produced by scripts/extract_frames_m2cai16.py):
      {data_path}/frames_hdf5/{video_id}.h5
        /frames            (N, H, W, 3)  uint8 RGB  — raw 1fps frames
        /frames_cutmargin  (N, 250, 250, 3) uint8 RGB — black-margin cropped
    """
        ...
        for video_id in infos.keys():
            for line_info in infos[video_id]:
                ...
                hdf5_path = os.path.join(
                    self.data_path, "frames_hdf5", line_info["video_id"] + ".h5"
                )
                line_info["hdf5_path"] = hdf5_path
                line_info["img_path"] = hdf5_path  # kept for error messages
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        sampled_image_list, sampled_label_list = [], []
        ...
        return np.stack(sampled_image_list), np.stack(sampled_label_list), sampled_list
