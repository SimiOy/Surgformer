...
class PhaseDataset_LungRes80(Dataset):
    """Load video phase recognition dataset."""

    def __init__(
        self,
        anno_path="/home/ultraabl/Surgformer/LungRes80/labels_pkl/train/1fps_train.pickle",
        data_path="/home/ultraabl/Surgformer/LungRes80",
        mode="train",  # val/test
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                img_path = os.path.join(
                    ...
                    "frames",
                    line_info["video_id"],
                    str(line_info["original_frame_id"]).zfill(6) + ".jpg"
                    if "original_frame_id" in line_info
                    else str(line_info["frame_id"]).zfill(6) + ".jpg",
                )
...
def build_dataset(is_train, test_mode, fps, args):
    if args.data_set == "LungRes80":
        ...
        dataset = PhaseDataset_LungRes80(
            anno_path=anno_path,
